...
export function RestaurantDetailPage() {
  ...
  const openReservations = reservations.filter((item) => item.restaurantId === id)
  return <section className="page-container page-section"><div className="grid gap-8 lg:grid-cols-[1.45fr_.75fr]">
    ...
    <aside className="lg:sticky lg:top-28 lg:self-start"><div className="card p-6 shadow-card"><p className="text-sm font-semibold text-brand">밥 모임</p><h2 className="mt-2 text-xl font-semibold">같이 먹을 사람을 찾아보세요</h2><p className="mt-3 text-sm leading-6 text-muted">원하는 시간이 없다면 직접 새 모임을 만들 수 있어요.</p><Link to={`/restaurants/${id}/reservations/new`}><Button fullWidth className="mt-6">새 모임 만들기</Button></Link></div>{openReservations.map((item) => <div key={item.id} className="card mt-4 p-5"><p className="text-sm font-semibold">{formatDateTime(item.dateTime)}</p><p className="mt-2 text-sm text-muted">{item.note}</p><div className="mt-4 flex items-center justify-between"><span className="flex items-center gap-1 text-xs text-muted"><Users size={14} />{item.joined}/{item.capacity}명</span><Button variant="secondary" className="h-10 px-4">참여하기</Button></div></div>)}</aside>
  </div></section>
}
