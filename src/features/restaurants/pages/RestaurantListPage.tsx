...
export function RestaurantListPage() {
  ...
  const filtered = useMemo(() => data.filter((item) => (category === '전체' || item.category === category) && `${item.name} ${item.area} ${item.tags.join(' ')}`.includes(keyword)), [category, data, keyword])
  return <section className="page-container page-section"><PageHeader eyebrow="RESTAURANTS" title="어디서 함께 먹을까요?" description="원하는 지역과 메뉴를 찾아보고, 열려 있는 식사 모임에 참여해 보세요." />
    ...
    {isLoading ? <p className="py-20 text-center text-muted">식당을 불러오는 중입니다.</p> : <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{filtered.map((restaurant) => <RestaurantCard key={restaurant.id} restaurant={restaurant} />)}</div>}
  </section>
}
