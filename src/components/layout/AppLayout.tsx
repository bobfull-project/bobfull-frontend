    ...
    <header className="sticky top-0 z-30 border-b border-line bg-white/95 backdrop-blur">
      <div className="page-container flex h-18 items-center justify-between py-4">
        <div className="flex items-center gap-2">
          ...
          {apiConfig.useMock && <Link to="/dev/payment-test" className="flex items-center gap-1.5 rounded-full border border-line bg-white px-3 py-2 text-xs font-semibold text-brand transition hover:bg-brand-soft" title="백엔드가 발급한 실제 paymentId/amount로 PortOne 결제창을 여는 수동 테스트 페이지입니다."><CreditCard size={14} /><span className="hidden sm:inline">결제 수동 테스트</span><span className="sm:hidden">수동 테스트</span></Link>}
        </div>
